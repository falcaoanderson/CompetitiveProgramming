...
void dfs_ciclo(int i){
    ...
    if(pos_ciclo[filho[i]]==filho[i]){ // se o filho ja foi visitado ele eh o comeco do ciclo
        dist_ciclo[i] = 1;
        return;
    }
...
void dfs_per(int i){
    ...
    if(qnt_pais[i]>1){ // se esse vertice faz parte do ciclo, significa que sai da periferia
        if(pos_ciclo[i]==0) dfs_ciclo(i); // se o ciclo ainda nao foi visitado eu faco
        return;
    }
