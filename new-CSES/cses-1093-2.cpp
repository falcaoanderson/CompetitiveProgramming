// 24/04/23 // dp com otimizacao de memoria
#include <iostream>
#include <algorithm>
#include <vector>
#include <queue>
...
int main(){
    ...
    for(int i=1; i<=n; i++){
        for(int j=0; j<=sum; j++){
            solve[i&1][j] = solve[~i&1][j];

            if(j>=i) solve[i&1][j] = (solve[i&1][j] + solve[~i&1][j-i])%MOD;
        }
    }

    cout << (solve[n&1][sum] * expo(2, MOD-2, MOD))%MOD << endl;
